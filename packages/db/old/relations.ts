...
export const projectRelations = relations(project, ({ one, many }) => ({
  ...
  projectQueueJobs: many(projectQueueJob),
...
export const projectQueueJobRelations = relations(
  projectQueueJob,
  ({ one }) => ({
    project: one(project, {
      fields: [projectQueueJob.projectId],
      references: [project.id],
    }),
    queueJob: one(queueJobs, {
      fields: [projectQueueJob.queueJobId],
      references: [queueJobs.id],
    }),
  }),
);

export const queueJobsRelations = relations(queueJobs, ({ many }) => ({
  projectQueueJobs: many(projectQueueJob),
}));
